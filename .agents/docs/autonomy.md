...
- a change would cross a `[VOUCHED @hyf0]` decision in decisions.md (currently
  **D1** not-configurable, **D2** zero-sudo, **D7** terminal takeover, **D9** Everforest visual identity,
  **D11** truecolor color-tier fallback, **D13** `npx machud` zero-install distribution, **D14** no
  accessibility layer, **D15** the two-tone `mac|hud` wordmark) — these are settled;
